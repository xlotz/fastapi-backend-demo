...
class CustomOAuth2PasswordRequestForm(OAuth2PasswordRequestForm):
    """
    自定义OAuth2PasswordRequestForm类，增加验证码及会话编码参数
    """
...
class LoginService:
    ...
    async def authenticate_user(cls, request: Request, db: AsyncSession, login_user: UserLogin):
        ...
        if login_user.user_name == account_lock:
            logger.warning('账户已锁定，请稍后再试')
            raise LoginException(data='', message='账户已锁定，请稍后再试')

        # 判断请求是否来自api文档，如果是，返回指定格式结果
        ...
        # 判断是否开启验证码， dev下api文档的登录不验证
        if not login_user.captcha_enabled or (
                (request_from_swagger or request_from_redoc) and AppConfig.app_dev == 'dev'
        ):
            pass
        else:
        ...
        if not PwdUtil.verify_password(login_user.password, user[0].password):
            ...
            #     )
            #     logger.warning('10分钟内密码错误5次，账户已锁定，请10分钟后再试')
            #     raise LoginException(data='', message='10分钟内密码错误5次，账户已锁定，请10分钟后再试')
            logger.warning('密码错误')
    ...
    async def get_current_user(
        ...
        query_user = await UserDao.get_user_by_id(db, user_id=token_data.user_id)
    ...
    @classmethod
    async def get_current_user_routers(cls, user_id: int, db: AsyncSession):
        """
        根据用户id获取当前用户路由信息
    ...
    @classmethod
    async def register_user_services(cls, request: Request, db: AsyncSession, user_register: UserRegister):
        """
        用户注册services
    ...
    @classmethod
    async def get_sms_code_services(cls, request: Request, db: AsyncSession, user: ResetUserModel):
        """
        获取短信验证码service
        ...
        is_user = await UserDao.get_user_by_name(db, user.user_name)
        if is_user:
