...
from sub_applications.exceptions.exception import LoginException, AuthException, ServiceException
from module_admin.dao.login_dao import login_by_account
from module_admin.dao.user_dao import UserDao
from module_admin.entity.do.menu_do import SysMenu
from module_admin.entity.vo.common_vo import CrudResponseModel
from module_admin.entity.vo.login_vo import MetaModel, MenuTreeModel, RouterModel, UserLogin
...
class CustomOAuth2PasswordRequestForm(OAuth2PasswordRequestForm):
    """
    自定义OAuth2PasswordRequestForm类，增加验证码及会话编号
    """

    def __init__(
            self, grant_type: str = Form(default=None, regex='password'),
...
class LoginService:
    ...
    """
    @classmethod
    async def authenticate_user(cls, request: Request, db: AsyncSession, login_user: UserLogin):
        """
        根据用户名密码登录
        :param request:
        :param db:
        ...
        #     logger.warning('账户已被锁定，请稍后再试')
        #     raise LoginException('账户已被锁定，请稍后再试')
        ...
        # # 验证码是否开启验证
        # if not login_user.captcha_enabled or (request_from_docs or request_from_redoc) and AppConfig.app_env == 'dev':
        #     pass
        # else:
        #     await cls.__check_login_captcha(request, login_user)

        user = await login_by_account(db, login_user.user_name)
        if not user:
        ...
        if not PwdUtil.verify_password(login_user.password, user[0].password):
            ...
            #     )
            #
            #     logger.warning('10分组内密码已错5次，账户已锁定，请10分钟后再试')
            #     raise LoginException(data='', message='10分组内密码已错5次，账户已锁定，请10分钟后再试')
            logger.warning('密码错误')
            return LoginException(data='', message='密码错误')
    ...
    async def get_current_user(
            cls, request: Request = Request, token: str = Depends(oauth2_scheme), db: AsyncSession = Depends(get_db)
    ):
        """
        根据token获取当前用户信息
        :param request:
        ...
        :return:
        """
        logger.info(f'login_services token--------- {token}')
        try:
            ...
            logger.warning('用户token失效，请重新登录')
            raise AuthException(data='', message='用户token失效，请重新登录')

        query_user = await UserDao.get_user_by_id(db, token_data.user_id)
    ...
    @classmethod
    async def get_current_user_routers(cls, user_id, db: AsyncSession):
        """
        根据用户id获取当前用户路由信息
        :param user_id:
        :param db:
        ...
        query_user = await UserDao.get_user_by_id(db, user_id)
        ...
        router = [router.model_dump(exclude_unset=True, by_alias=True) for router in user_router]
        return router
    ...
    def __generate_user_router_menu(cls, permission_list: List[MenuTreeModel]):
        ...
        logger.info(f'-------- login_services: permission_list: {permission_list}')
        for permission in permission_list:
            router = RouterModel(
    ...
    @classmethod
    async def forget_user_services(cls, request: Request, db: AsyncSession, forget_user: ResetUserModel):
        """
        用户忘记密码services
        ...
        if forget_user.sms_code == redis_sms_result:
            ...
            forget_user.user_id = (await UserDao.get_user_by_name(db, forget_user.user_name)).user_id
            edit_result = await UserService.reset_user_services(db, forget_user)
